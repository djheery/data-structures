...
As you can see rather than checking the value of the `deleted_data` against `x->left/right->key` as we do insertion. 
...
Below is the example code for AVL tree delete:

```
Node* delete(AVLTree* tree, Node* root, unit32_t search_data) {
    if (root == NULL) return root; 

    if (search_data < root->data) {
        root->left = delete(tree, root->left, search_data);
    } else if (search_data > root->data) {
        root->right = delete(tree, root->right, search_data); 
    } else {
